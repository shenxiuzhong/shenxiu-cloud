...
public interface SysDictTypeApi {
     ...
     */
    @GetMapping("/getPage")
    ResEntity<PageData<SysDictType>> getPage(SysDictType dictType, PageQuery pageQuery);
    ...
    ResEntity<SysDictType> getInfo(@PathVariable Long dictId);
    ...
    ResEntity<Void> add(@Validated @RequestBody SysDictType dict);
    ...
    ResEntity<Void> edit(@Validated @RequestBody SysDictType dict);
     ...
     */
    @PostMapping("/remove")
    ResEntity<Void> remove(@RequestBody Long[] dictIds);
    ...
    ResEntity<List<SysDictType>> optionselect();
}
